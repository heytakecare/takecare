...
let ImagePlacer = function() {
  this.container = document.getElementById('images')
  this.placedImages = []
}

ImagePlacer.prototype.images = function() {
  if (this.container != undefined) {
    return JSON.parse(this.container.dataset.images)
  }
}

ImagePlacer.prototype.prompt = function() {
  let placedImages = this.placedImages
  window.setTimeout(function() {
    if(!placedImages.length) {
      console.log("Place an image")
    }
  }, 5000)
}

ImagePlacer.prototype.place = function() {
    let imagePlacer = this
    ...
    this.container.onclick = function(event) {
      ...
      imagePlacer.placedImages.push(imgData)
      img.src = imgData.src
